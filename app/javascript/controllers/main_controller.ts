...
export default class extends Controller {
  ...
  declare indicesGridTarget: HTMLElement
  declare hasIndicesGridTarget: boolean
  declare forexGridTarget: HTMLElement
  declare hasForexGridTarget: boolean
  declare commoditiesGridTarget: HTMLElement
  declare hasCommoditiesGridTarget: boolean
  ...
  _closeDropdown = (e: MouseEvent) => {
    if (!(e.target as HTMLElement).closest("[data-add-tile-dropdown]")) {
  ...
  async showAddDropdown(e: Event) {
    ...
    try {
      ...
      const available = config.symbols.filter((s: string) => !displayed.includes(s))
  ...
  _renderMarketSection(targetName: string, quotes: unknown[], category: string): void {
    const target = (this as any)[`has${targetName.charAt(0).toUpperCase() + targetName.slice(1)}Target`]
      ? (this as any)[`${targetName}Target`] : null
    if (!target) return
    target.innerHTML = (quotes || []).map((q: any) => marketTileHTML(q, category)).join("")
  }

  async removeMarket(e: Event) {
    e.stopPropagation()
    const ds = (e.currentTarget as HTMLElement).dataset
    await this._postAndRefresh("/api/markets/remove", { symbol: ds.symbol!, category: ds.category! }, "remove instrument", () => this._fetchMarkets())
  }

  async showAddMarketDropdown(e: Event) {
    e.stopPropagation()
    ...
    const displayed = section ? Array.from(section.querySelectorAll("[data-symbol]")).map(el => (el as HTMLElement).dataset.symbol) : []
    const remaining = available.filter((s: string) => !displayed.includes(s))

    this._openDropdown(btn, remaining, "click->main#addMarket", { labels, extraAttrs: `data-category="${category}"` })
  }

  async addMarket(e: Event) {
    e.stopPropagation()
    this._dismissDropdown()
    const ds2 = (e.currentTarget as HTMLElement).dataset
    await this._postAndRefresh("/api/markets/add", { symbol: ds2.symbol!, category: ds2.category! }, "add instrument", () => this._fetchMarkets())
  }

  // --- Shared helpers ---

  async _postAndRefresh(url: string, body: Record<string, string | undefined>, action: string, refreshFn: () => void): Promise<void> {
  ...
  _openDropdown(btn: HTMLElement, items: string[], action: string, opts?: { labels?: Record<string, string>; extraAttrs?: string }): void {
    const { labels = {}, extraAttrs = "" } = opts ?? {}
