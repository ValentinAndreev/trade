...
export default class VolumeProfileManager {
  ...
  get enabled(): boolean { return this._vpEnabled }
...
export function computeVolumeProfile(candles: Candle[], numRows: number): { price: number; volume: number }[] {
  if (!candles || candles.length === 0) return []
