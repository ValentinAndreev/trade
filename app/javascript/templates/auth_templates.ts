...
export function userAreaHTML(username: string, presetName?: string): string {
...
export function loginButtonHTML(): string {
...
export function presetsMenuHTML(
  presets: Array<{ id: number; name: string; is_default?: boolean; updated_at: string }>,
...
export function savePresetFormHTML(defaultName: string): string {
  return `
...
export function presetPickerHTML(
  presets: Array<{ id: number; name: string; is_default?: boolean }>
): string {
  const rows = presets.map(p => `
    <button data-action="click->auth#applyPreset" data-preset-id="${p.id}" data-preset-name="${p.name}"
