...
function drawingListHeaderHTML(ctrl, toggleAction, clearAction, headerLabel, collapsed, count) {
  return `
...
function drawingItemHTML(ctrl, kind, item, opts) {
  const {
    nameFn, subtextFn, timeFn, defaultColor, defaultWidth,
    hasWidthPicker, hasFontSizePicker, hasSelect, hasRename, colorIndicatorFn,
  } = opts
...
function fontSizePickerHTML(ctrl, kind, id, fontSize, enabled) {
  if (!enabled) return ""
...
function widthPickerHTML(ctrl, kind, id, width, enabled) {
  if (!enabled) return ""
  return `
