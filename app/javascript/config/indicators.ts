...
const v = (d: CandleWithValue[]): number[] => d.map(x => x.value)
const hi = (d: CandleWithValue[]): number[] => d.map(x => x.high)
const lo = (d: CandleWithValue[]): number[] => d.map(x => x.low)
const cl = (d: CandleWithValue[]): number[] => d.map(x => x.close)
const vol = (d: CandleWithValue[]): number[] => d.map(x => x.volume)
