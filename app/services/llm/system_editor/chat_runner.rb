  ...
  module SystemEditor
    class ChatRunner
      ...
      def call(content:, editor_context:)
        ...
        agent = Llm::SystemEditorAgent.new(chat: @chat, editor_context: normalized_context, persist_instructions: false)
        ...
        Result.new(chat: @chat, assistant_message: assistant_message)
      end
      ...
      def persist_draft_metadata(message, draft)
        message.update!(metadata: message.metadata.merge('draft' => draft))
      end
    end
  end
end
