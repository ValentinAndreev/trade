...
- [ ] **ac-cap-grid-prediction-cells:** Backend rejects requests above 50,000 total prediction cells using `candle_count * distinct(modelKey, modelOutput)`, and frontend preflight prevents obvious over-cap requests with the same formula. Error hints include the deduplicated model/output count and do not reuse the 017 single-model `distinct_requested_models: 1` hint.
