...
3. Backend cap follows the 017 single-market formula: `MAX_PREDICTION_CELLS = 50_000`, where `cell_count = candle_count * distinct(modelKey, modelOutput)` requested serving series for one `(exchange, symbol, timeframe)` tuple. Each `ml_prediction` column renders exactly one `modelOutput`; duplicate columns for the same pair do not require duplicate inference.
4. Frontend preflights the same distinct-pair cell count and shows a disabled/error state before sending obviously over-cap requests. The backend exposes the cap value used by the UI so frontend/backend limits do not drift silently. Visible-range prediction requests are debounced/throttled, stale in-flight responses are ignored, and the backend keeps an authenticated-session guard for concurrent grid prediction requests so a frontend bug cannot create unbounded in-process inference work.
5. Partial prediction responses are valid: available timestamps render values and missing timestamps render `nil`. Sorting an ML column places `nil` values last in both ascending and descending order; numeric range filters exclude `nil` values unless a later explicit missing-value filter is added.
...
10. Model autocomplete uses capped `/api/ml/models` responses with a prefix query parameter and a `truncated`/`has_more` indicator; the UI prompts the user to refine the query instead of silently hiding matches beyond the first 50.
...
- [ ] **ac-cap-grid-prediction-cells:** Backend rejects requests above 50,000 total prediction cells using `candle_count * distinct(modelKey, modelOutput)`, and frontend preflight prevents obvious over-cap requests with the same formula.
...
- [ ] **ac-handle-partial-ml-grid-results:** Data-grid ML columns render available prediction timestamps, render `nil` for missing timestamps, place `nil` values last when sorting ML columns, exclude `nil` from numeric range filters, and keep sorting/filtering usable for the rest of the rows.
...
- [ ] **ac-model-key-autocomplete:** System editor autocomplete fetches available model keys through capped model API responses with prefix search and a truncated/has-more indicator, and never loads weight blobs.
