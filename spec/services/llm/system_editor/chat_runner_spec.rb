...
RSpec.describe Llm::SystemEditor::ChatRunner do
  ...
  let(:chat) { user.ai_chats.create!(title: 'New chat', source_path: 'systems/alpha.yml', system_id: 'alpha') }
  ...
  describe '#call' do
    ...
    it 'uses the tool agent for tool-capable remote models' do
      ...
      agent = instance_double(Llm::SystemEditorAgent, ask: nil)
      agent_chat = instance_double(AiChat, with_temperature: nil)
      allow(agent).to receive(:chat).and_return(agent_chat)
      allow(Llm::SystemEditorAgent).to receive(:new).and_return(agent)
      ...
      described_class.new(user:, chat:, setting:).call(content: 'Fix the exits', editor_context:)

      expect(Llm::SystemEditorAgent).to have_received(:new)
    ...
    it 'falls back to a plain chat without tools for local endpoints' do
      ...
      allow(Llm::SystemEditorAgent).to receive(:new)

      result = described_class.new(user:, chat:, setting:).call(content: 'Fix the exits', editor_context:)

      expect(Llm::SystemEditorAgent).not_to have_received(:new)
    ...
    it 'uses the OpenAI-compatible runtime for llama.cpp provider' do
      ...
      described_class.new(user:, chat:, setting:).call(content: 'What model are you?', editor_context:)
