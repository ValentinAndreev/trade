...
RSpec.describe CandleSyncJob do
  ...
  it 'calls Candle::Syncer for each symbol' do
    BitfinexConfig.symbols.each do |symbol|
      fetcher = instance_double(Candle::Syncer)
      allow(Candle::Syncer).to receive(:new).with(symbol).and_return(fetcher)
    ...
    BitfinexConfig.symbols.each do |symbol|
      expect(Candle::Syncer).to have_received(:new).with(symbol)
    end
  end
  ...
  it 'continues syncing remaining symbols when one fails' do
    ...
    symbols.each do |symbol|
      ...
      if symbol == failing_symbol
        allow(fetcher).to receive(:call).and_raise(Candle::Sync::FetchError, 'boom')
      else
        allow(fetcher).to receive(:call)
      end
    end
  ...
  it 'skips sync and broadcasts when Bitfinex is unreachable' do
    ...
    allow(Candle::Syncer).to receive(:new)

    described_class.perform_now

    expect(Candle::Syncer).not_to have_received(:new)
